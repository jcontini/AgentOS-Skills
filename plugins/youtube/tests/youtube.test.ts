 ...
 * Run with: npm test -- plugins/youtube/tests/youtube.test.ts
 * 
...
async function callPlugin(tool: string, params: Record<string, unknown>): Promise<unknown> {
  const response = await fetch(`${BASE_URL}/api/plugins/youtube/${tool}`, {
    method: 'POST',
    headers: {
...
describe('YouTube', () => {
  ...
  it('video.get returns video metadata', async () => {
    const result = await callPlugin('video.get', { url: TEST_VIDEO }) as Record<string, unknown>;
  ...
  it('video.search returns array of videos', async () => {
    const result = await callPlugin('video.search', { query: 'programming tutorial' }) as Array<Record<string, unknown>>;
  ...
  it('video.search_recent returns recent videos', async () => {
    const result = await callPlugin('video.search_recent', { query: 'news today' }) as Array<Record<string, unknown>>;
  ...
  it('video.list returns videos from channel', async () => {
    const result = await callPlugin('video.list', { url: TEST_CHANNEL }) as Array<Record<string, unknown>>;
  ...
  it('video.transcript returns transcript text', async () => {
    const result = await callPlugin('video.transcript', { url: TEST_VIDEO_WITH_CAPTIONS }) as Record<string, unknown>;
