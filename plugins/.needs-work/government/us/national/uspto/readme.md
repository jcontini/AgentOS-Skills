...
2. `~/.agentos/integrations/CONTRIBUTING.md` — How apps/plugins work
...
**The solution:** Create an IP (Intellectual Property) app with USPTO as the first plugin. Supports both trademark (TSDR) and patent (Open Data Portal) searches.
...
└─────────────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│  USPTO Connector (apps/ip/plugins/uspto/)                         │
...
instructions: |
  ...
  **Getting started:**
  ...
  2. Search trademarks: `IP(action: "search_trademarks", plugin: "uspto", params: {query: "APPLE"})`
  3. Get trademark: `IP(action: "get_trademark", plugin: "uspto", params: {serial_number: "85123456"})`
...
```
IP(action: "search_trademarks", plugin: "uspto", params: {query: "APPLE"})
IP(action: "search_trademarks", plugin: "uspto", params: {query: "Nike", status: "registered"})
```

### Get Trademark Details

```
IP(action: "get_trademark", plugin: "uspto", params: {serial_number: "85123456"})
IP(action: "get_trademark", plugin: "uspto", params: {registration_number: "1234567"})
```

### Search Patents

```
IP(action: "search_patents", plugin: "uspto", params: {query: "machine learning"})
IP(action: "search_patents", plugin: "uspto", params: {assignee: "Apple Inc"})
```
...
Future plugins could include: EPO (European), WIPO (International), Google Patents.
```
...
United States Patent and Trademark Office plugin.
...
```bash
mkdir -p ~/.agentos/integrations/apps/ip/plugins/uspto
```
...
### Step 2: Create USPTO plugin (2 hours)

- `apps/ip/plugins/uspto/readme.md` — Auth + action implementations
- `apps/ip/plugins/uspto/icon.png` — USPTO logo
...
- [ ] Can search trademarks: `IP(action: "search_trademarks", plugin: "uspto", params: {query: "Nike"})`
- [ ] Can get trademark by serial: `IP(action: "get_trademark", plugin: "uspto", params: {serial_number: "..."})`
- [ ] Can get trademark by registration: `IP(action: "get_trademark", plugin: "uspto", params: {registration_number: "..."})`
- [ ] Can search patents: `IP(action: "search_patents", plugin: "uspto", params: {query: "AI"})`
- [ ] Can get patent: `IP(action: "get_patent", plugin: "uspto", params: {application_number: "..."})`
